...
public class SecurityHelper implements ISecurityHelper {
  ...
  /**
   * Utility method to determine whether a particular role is granted to the user.
   *
   * @param role
   ...
   */
  @Override
  public boolean isGranted( GrantedAuthority role ) {
    Authentication auth = getAuthentication();
    if ( auth != null && auth.isAuthenticated() && auth.getAuthorities() != null ) {
      GrantedAuthority[] userAuths = auth.getAuthorities().toArray( new GrantedAuthority[]{} );
      if ( userAuths == null ) {
        return false;
      }
