...
public class SpringSecurityJdbcAuthTest extends AbstractSpringSecurityAuth {
  ...
  @Override UserDetails getValidUser() {
    return new DummyUserDetails( "jdbcUser", "password", new String[] { role1.getName(), role2.getName() } );
  }
  ...
  @Override AuthenticationProvider getMockAuthenticationProvider() throws Exception {
    ...
    if( mockAuthProvider == null ){
       ...
       * they REALLY should've created a getter and called that one, instead of calling directly a final logger...
       */
  ...
  @Override UserDetailsService getMockUserDetailsService() throws Exception {
    ...
    if( mockUserDetailsService == null ) {
      ...
      mockUserDetailsService = mock( DefaultRoleJdbcDaoImpl.class );
